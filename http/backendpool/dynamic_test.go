...
func TestDynamic_Operations(t *testing.T) {
	...
	assert.NoError(t, d.AddOrUpdate(&pb.Backend{Name: "foobar", DisableConntracking: true}, false))
	assert.NoError(t, d.AddOrUpdate(&pb.Backend{Name: "carbar", DisableConntracking: true}, false))
	assert.Len(t, d.Configs(), 2, "we should have two")
	oldCarBar := d.backends["carbar"]
	assert.NoError(t, d.AddOrUpdate(&pb.Backend{Name: "carbar"}, false), "updating carbar shouldn't fail")
