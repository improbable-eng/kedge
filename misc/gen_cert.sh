...
openssl req -nodes -new -x509 -days 2048 -key ca.key -out ca.crt \
...
openssl x509 -req -days 2048 -in client.csr -CA ca.crt -CAkey ca.key -passin pass:foo -set_serial 01 -out client.crt
...
openssl x509 -req -days 2048 -in localhost.csr -CA ca.crt -CAkey ca.key -passin pass:foo -set_serial 10 -out localhost.crt
